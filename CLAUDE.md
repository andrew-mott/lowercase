...
- `packages/ports` — interfaces only (event bus, messaging and message log, artifact store/reader/writer/repo, run repo/query, flow/sim repo, event store, limiter, services). No implementations.
...
- `packages/components/*` — long-lived, self-driven by subscribing to the event bus rather than being called: `engine`, `worker`, `limiter`, `observability`.
- `packages/adapters` — concrete implementations of every port, including all Prisma-backed repositories (`prisma-run-repository.ts`, `prisma-artifact-repository.ts`, etc.) and non-SQL adapters (`FsArtifactStore`, `S3ArtifactStore`, `RedisMessageLog`, `InMemoryEventBus`).
